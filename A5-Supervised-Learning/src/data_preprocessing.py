...
class NotePreprocessing:
    def __init__(self, notes_file, admissions_file, max_features, ngram_range):
        ...
        if notes_file is None:
            ...
            notes = pd.read_csv(self.data_dir  / "NOTEEVENTS_subset.csv", engine='python', encoding='utf-8', error_bad_lines=False)
        ...
        if admissions_file is None:
            ...
            admissions = pd.read_csv(self.data_dir / "ADMISSIONS_subset.csv", engine='python', encoding='utf-8', error_bad_lines=False)
        ...
        (train, validation, test), (train_labels, valid_labels, test_labels) = self.tfidf_vectorisation(train_split_balanced, 
                                                                                                        valid_split, 
                                                                                                        test_split, 
                                                                                                        max_features, 
                                                                                                        ngram_range)        
    ...
    def merge_dataframes(self, admissions, discharge_sums):
    ...
    '''
    '''
    def tfidf_vectorisation(self, train_balanced, valid, test, max_features, ngram_range):
        ...
        valid_labels.to_csv(self.out_dir / "valid_labels.csv")
        ...
        pd.DataFrame(tfidf_vect_notes_valid_array).to_csv(self.out_dir / "tfidf_valid_notes.csv")

        pd.DataFrame(tfidf_vect_notes_test_array).to_csv(self.out_dir / "tfidf_test_notes.csv")

        return (tfidf_vect_notes_array, tfidf_vect_notes_valid_array, tfidf_vect_notes_test_array), (train_labels, valid_labels, test_labels)
